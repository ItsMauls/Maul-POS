...
type userRepository interface {
	...
	StoredAccessTokenToRedis(token string, userID int64) (err error)
	StoredRefreshTokenToRedis(token string, userID int64) (err error)
	GetAccessTokenFromRedis(token string) (userID int64, err error)
	GetRefreshTokenFromRedis(token string) (userID int64, err error)
...
func NewService(cfg *lib.Config, log *lib.AggregateLogger, repo userRepository) *service {
	return &service{
		cfg:  cfg,
		log:  log,
		repo: repo,
...
func (s *service) Login(username, password string) (res LoginResponse, err error) {
	...
	passwordHashed, err := lib.HashPassword(password)
	if err != nil {
		s.log.ErrorF("service.Login.HashPassword", err)
		return res, err
	}
