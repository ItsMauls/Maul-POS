...
func NewAuthRepository(log *lib.AggregateLogger, cfg *lib.Config, db *sqlx.DB, redis *redis.Pool) *repository {
	return &repository{
		log:   log,
		cfg:   cfg,
		db:    db,
...
func (r *repository) StoredAccessTokenToRedis(token string, userID int64) (err error) {
	...
	r.log.InfoF("repository.StoredAccessTokenToRedis", "token", token, "userID", userID)
	...
	if err != nil {
		r.log.ErrorF("repository.StoredAccessTokenToRedis", err)
		return err
	}
