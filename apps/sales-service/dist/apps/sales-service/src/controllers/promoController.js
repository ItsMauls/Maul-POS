...
exports.getPromos = exports.createPromo = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const httpStatus_1 = require("../constants/httpStatus");
const createPromo = async (req, res) => {
    const { nama, deskripsi, diskon, tanggalAwal, tanggalAkhir, jamAwalPromo, jamAkhirPromo, noPromo, jenisPromo, mainstockId, 
    // New fields for specific promo types
    minPembelian, maxDiskon, kuantitasBeli, kuantitasGratis } = req.body;
    try {
        let promoData = {
            nama,
            deskripsi,
        ...
        const promo = await prisma_1.default.promo.create({ data: promoData });
        res.status(httpStatus_1.HTTP_STATUS.CREATED).json(promo);
    }
    catch (error) {
        console.error('Error creating promo:', error);
        res.status(httpStatus_1.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to create promo' });
    }
};
...
const getPromos = async (req, res) => {
    try {
        ...
        res.status(httpStatus_1.HTTP_STATUS.OK).json(promos);
    }
    catch (error) {
        res.status(httpStatus_1.HTTP_STATUS.INTERNAL_SERVER_ERROR).json({ error: 'Failed to retrieve promos' });
    }
};
